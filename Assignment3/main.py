...
meal_window = meal_window.loc[
    (meal_window["shiftUp"] > datetime.timedelta(minutes=120))
    | (pd.isnull(meal_window["shiftUp"]))
]
...
for i in range(len(meal_window)):
    meal_window_before = meal_window["dateTime"].iloc[i] - datetime.timedelta(
        minutes=30
    )
    meal_window_after = meal_window["dateTime"].iloc[i] + datetime.timedelta(
        minutes=120
    )
    meal_window_cgm_data = cgm_data.loc[
        (cgm_data["dateTime"] >= meal_window_before)
        & (cgm_data["dateTime"] < meal_window_after)
    ]
...
meal_data_cgm.dropna(
    axis=0, how="all", thresh=total_columns / 4, subset=None, inplace=True
)
meal_data_cgm.dropna(
    axis=1, how="all", thresh=total_rows / 4, subset=None, inplace=True
)
meal_data_cgm.interpolate(
...
meal_data_cgm = pd.merge(
    meal_data_cgm, meal_data_cgm_idx, left_index=True, right_index=True
)
meal_data_cgm["mean CGM data"] = no_meal_data_cgm_idx.mean(axis=1)
meal_data_cgm["max-start_over_start"] = (
    no_meal_data_cgm_idx.max(axis=1) / no_meal_data_cgm_idx[0]
)
...
meal_qty_lbl["Bin Label"] = meal_qty.apply(
    lambda row: get_bin_lbl(row["Meal Amount"]).astype(np.int64), axis=1
)
...
meal_data_qty = meal_data_qty.merge(
    meal_carbs_intake_period, how="inner", on=["New Index"]
)
...
bins_clusters_df = pd.DataFrame(
    {"ground_true_arr": groundTruth_data, "kmeans_labels": list(y_val_predict)},
...
confusion_matrix_copy["Row_entropy"] = confusion_matrix.apply(
    lambda row: get_entropy(row), axis=1
)
data_sum = confusion_matrix_copy["Total"].sum()
confusion_matrix_copy["entropy_prob"] = (
    confusion_matrix_copy["Total"] / data_sum * confusion_matrix_copy["Row_entropy"]
)
...
dbscan_bins_clusters = pd.DataFrame(
    {"ground_true_arr": groundTruth_data, "dbscan_labels": list(model.labels_)},
    columns=["ground_true_arr", "dbscan_labels"],
)

dbscan_confusion_matrix = pd.pivot_table(
    dbscan_bins_clusters, index="ground_true_arr", columns="dbscan_labels", aggfunc=len
)
...
dbscan_confusion_matrix_copy["Row_entropy"] = dbscan_confusion_matrix.apply(
    lambda row: get_entropy_dbscan(row), axis=1
)
data_sum = dbscan_confusion_matrix_copy["Total"].sum()
dbscan_confusion_matrix_copy["entropy_prob"] = (
    dbscan_confusion_matrix_copy["Total"]
    / data_sum
...
for i in range(len(centroid_features_dbscan)):
    centroid_features_dbscan["centroid_difference"].iloc[i] = math.pow(
        ...
        - centroid_features_dbscan["centroid_carb_input"].iloc[i],
        2,
    ) + math.pow(
        centroid_features_dbscan["mean CGM data"].iloc[i]
        - centroid_features_dbscan["centroid_cgm_mean"].iloc[i],
        2,
    )

for i in range(len(dbscan_confusion_matrix.columns)):
    squared_error[i] = centroid_features_dbscan.loc[
        centroid_features_dbscan["cluster"] == i
